...
async def get_developers_name(request: Request, db: AsyncSession = Depends(get_db)):
...
@limiter.limit("10/minute")
async def get_developer_by_id(request: Request, developer_id: int, db: AsyncSession = Depends(get_db), redis=Depends(get_redis)):
...
async def delete_developer(request: Request, developer_id: int, db: AsyncSession = Depends(get_db), current_user: Developer = Depends(get_current_user)):
...
@limiter.limit("5/minute")
async def update_developer(request: Request, developer_id: int, developer_create: DeveloperCreate, db: AsyncSession = Depends(get_db), current_user: Developer = Depends(get_current_user), redis=Depends(get_redis)):
    service = DeveloperService(db, redis)
